...
for episode in range(num_episodes):
    ...
    while not terminated and not truncated:
        ...
        if len(buffer) >= batch_size:
            ...
            states = torch.FloatTensor(np.array([exp[0] for exp in sample_batch]))
            actions = torch.LongTensor(np.array([exp[1] for exp in sample_batch]))
            rewards = torch.FloatTensor(np.array([exp[2] for exp in sample_batch]))
            next_states = torch.FloatTensor(np.array([exp[3] for exp in sample_batch]))
            dones = torch.FloatTensor(np.array([exp[4] for exp in sample_batch]))
...
print(
    f"MODEL SAVED: weights/dqn_seed{seed}.pt"
)
